/*
  3-Button, 3-Song Buzzer Player (blocking playback)

  Behavior:
  - Press A1: toggles Song 1
  - Press A2: toggles Song 2
  - Press A3: toggles Song 3
  - If a song is playing and you press its same button again: song stops immediately.
  - If a different song button is pressed while a song is playing: it switches to the new song.

  Notes:
  - Uses internal pullups. Wire each button from Ax pin to GND.
    When pressed, pin reads LOW.
  - Passive buzzer on D9 (change BUZZ_PIN if needed).
  - Songs are stored in PROGMEM arrays you will paste in (3 separate chunks).

  IMPORTANT:
  - Each song must define:
...
// Buttons (analog pins used as digital inputs)
const uint8_t BTN1_PIN = A1;
const uint8_t BTN2_PIN = A2;
const uint8_t BTN3_PIN = A3;

// Playback timing tweaks
const uint16_t SPEED_PCT = 100;    // 100 = as encoded
const uint8_t  EXTRA_GAP_MS = 2;   // tiny gap after each event

// Debounce
const uint16_t DEBOUNCE_MS = 35;

// =======================================================
// >>>>>> PASTE SONG 1 ARRAYS HERE <<<<<<
// =======================================================
// Generated from: David_Bowie_-_Space_Oddity.mid
// Track: Choir
// Total events (notes + rests): 328
...
static uint32_t lastDebounceMs1 = 0;
static uint32_t lastDebounceMs2 = 0;
static uint32_t lastDebounceMs3 = 0;
...
// Reads a button and returns true only on a debounced PRESS edge.
static bool pressedEdge(uint8_t pin, bool &lastState, uint32_t &lastDebounceMs) {
  bool reading = (digitalRead(pin) == LOW); // pressed = true
  bool lastReading = (lastState == LOW);
...
// Toggle logic when user presses a song button
static void handleSongButton(SongId requested) {
  ...
  // If currently playing the same song: stop
  ...
  // If playing a different song: switch
  requestStop();
  currentSong = requested;
  stopRequested = false;
  isPlaying = true;
...
// Blocking playback routine that still checks for stop requests between events
static void playSongBlocking(
  ...
  for (uint16_t i = 0; i < nEvents; i++) {
    ...
    if (freq == 0) {
      noTone(BUZZ_PIN);
      // During delays, poll buttons quickly so "stop" works responsively
      uint32_t start = millis();
      while (!stopRequested && (millis() - start) < dur) {
        // button polling
        if (pressedEdge(BTN1_PIN, lastBtn1, lastDebounceMs1)) handleSongButton(SONG_1);
        if (pressedEdge(BTN2_PIN, lastBtn2, lastDebounceMs2)) handleSongButton(SONG_2);
        if (pressedEdge(BTN3_PIN, lastBtn3, lastDebounceMs3)) handleSongButton(SONG_3);
        delay(1);
      }
      continue;
    }
    ...
    if (gate > 0) {
      tone(BUZZ_PIN, freq);
      uint32_t start = millis();
      while (!stopRequested && (millis() - start) < gate) {
        if (pressedEdge(BTN1_PIN, lastBtn1, lastDebounceMs1)) handleSongButton(SONG_1);
        if (pressedEdge(BTN2_PIN, lastBtn2, lastDebounceMs2)) handleSongButton(SONG_2);
        if (pressedEdge(BTN3_PIN, lastBtn3, lastDebounceMs3)) handleSongButton(SONG_3);
        delay(1);
    ...
    // Silence for remainder
    uint16_t offMs = dur - gate;
    if (offMs > 0) {
      uint32_t start = millis();
      while (!stopRequested && (millis() - start) < offMs) {
        if (pressedEdge(BTN1_PIN, lastBtn1, lastDebounceMs1)) handleSongButton(SONG_1);
        if (pressedEdge(BTN2_PIN, lastBtn2, lastDebounceMs2)) handleSongButton(SONG_2);
        if (pressedEdge(BTN3_PIN, lastBtn3, lastDebounceMs3)) handleSongButton(SONG_3);
        delay(1);
    ...
    if (EXTRA_GAP_MS && !stopRequested) {
      uint32_t start = millis();
      while (!stopRequested && (millis() - start) < EXTRA_GAP_MS) {
        if (pressedEdge(BTN1_PIN, lastBtn1, lastDebounceMs1)) handleSongButton(SONG_1);
        if (pressedEdge(BTN2_PIN, lastBtn2, lastDebounceMs2)) handleSongButton(SONG_2);
        if (pressedEdge(BTN3_PIN, lastBtn3, lastDebounceMs3)) handleSongButton(SONG_3);
        delay(1);
...
void loop() {
  // Idle: wait for a button press to start playback
  if (!isPlaying) {
    if (pressedEdge(BTN1_PIN, lastBtn1, lastDebounceMs1)) handleSongButton(SONG_1);
    if (pressedEdge(BTN2_PIN, lastBtn2, lastDebounceMs2)) handleSongButton(SONG_2);
    if (pressedEdge(BTN3_PIN, lastBtn3, lastDebounceMs3)) handleSongButton(SONG_3);
  ...
  // Play selected song to completion (unless stopped/toggled)
  switch (currentSong) {
    case SONG_1:
