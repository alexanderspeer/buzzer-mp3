...
David_Bowie_-_Space_Oddity.buzzer.json  (selected_track_name: "Choir")
Source data: precomputed event durations in milliseconds + gate times.

Hardware:
- Passive buzzer on D9
  D9 -> 300Ω -> buzzer+ ; buzzer- -> GND
...
const uint16_t SONG_FREQ_HZ[] PROGMEM = {
  ...
  0, 349, 0, 349, 0, 349, 0, 349, 0, 349, 0, 392, 0, 330, 0, 330,
  0, 330, 0, 349, 0, 349, 0, 349, 0, 349, 0, 349, 0, 349, 0, 392,
  0, 330, 0, 349, 0, 330, 0, 294, 0, 349, 0, 330, 294, 0, 262, 0,
  220, 0, 262, 0, 247, 0, 220, 0, 196, 0, 196, 0, 196, 220, 0, 196,
  0, 147, 0, 262, 0, 262, 0, 247, 220, 196, 0, 233, 0, 233, 0, 233,
  0, 262, 0, 220, 0, 220, 0, 220, 0, 196, 0, 196, 220, 0, 196, 175,
  0, 175, 0, 330, 0, 330, 0, 330, 0, 349, 0, 392, 0, 392, 0, 392,
  0, 349, 0, 330, 0, 330, 0, 330, 0, 330, 0, 349, 0, 349, 0, 349,
  0, 349, 0, 349, 0, 349, 0, 392, 0, 330, 0, 349, 0, 330, 0, 330,
  0, 330, 0, 349, 0, 349, 0, 349, 0, 349, 0, 349, 0, 392, 0, 330,
  0, 330, 0, 330, 0, 349, 0, 349, 0, 349, 0, 349, 0, 349, 0, 392,
  0, 330, 0, 330, 0, 330, 0, 349, 0, 349, 0, 349, 0, 349, 0, 349,
  0, 392, 0, 330, 0, 349, 0, 330, 0, 294, 0, 349, 0, 330, 294, 0,
  262, 0, 220, 0, 262, 0, 247, 0, 220, 0, 196, 0, 196, 0, 196, 220,
  0, 196, 0, 147, 0, 262, 0, 262, 247, 0, 220, 196, 0, 233, 0, 233,
  0, 233, 0, 262, 0, 220, 0, 220, 0, 220, 0, 196, 0, 196, 0, 220,
  0, 196, 175, 0, 175
};

const uint16_t SONG_DUR_MS[] PROGMEM = {
  331, 106, 72, 145, 469, 179, 79, 140, 874, 2360, 2208, 290, 140, 256, 181, 251,
  186, 251, 179, 362, 72, 258, 181, 285, 145, 256, 179, 367, 68, 79, 138, 1889,
  31268, 220, 102, 469, 183, 362, 75, 106, 77, 577, 106, 111, 106, 584, 70, 145,
  ...
  72, 399, 36, 326, 106, 403, 41, 394, 34, 72, 145, 543, 109, 2176, 10428, 258,
  ...
  618, 41, 215, 143, 72, 2645, 28695, 106, 86, 460, 215, 365, 75, 104, 111, 546,
  ...
  75, 580, 70, 326, 109, 326, 111, 106, 104, 478, 181, 1884, 4203, 181, 34, 181,
  435, 41, 174, 34, 186, 32, 1492, 18478, 2572, 34, 401, 36, 360, 111, 68, 145,
  72, 111, 179, 183, 38, 288, 186, 1660, 510, 29, 2144, 217, 654, 32, 186, 2751,
...
const uint16_t SONG_GATE_MS[] PROGMEM = {
  ...
  0, 161, 165, 0, 259, 0, 1494, 0, 30, 0, 196, 589, 0, 167, 2476, 0, 104,
  0, 226, 0, 522, 0, 391, 0, 620, 0, 261, 0, 224, 0, 130, 0, 359,
  556, 0, 202, 122, 0, 2362
};

const uint16_t N_EVENTS = sizeof(SONG_FREQ_HZ) / sizeof(SONG_FREQ_HZ[0]);

static inline uint16_t scaleMs(uint16_t ms) {
  // scale by SPEED_PCT (integer, rounded)
  uint32_t v = (uint32_t)ms * (uint32_t)SPEED_PCT + 50;
  v /= 100;
  if (v > 65535UL) v = 65535UL;
  return (uint16_t)v;
}

static inline uint16_t applyTranspose(uint16_t freqHz) {
  if (freqHz == 0 || TRANSPOSE == 0) return freqHz;

  int8_t t = TRANSPOSE;
  if (t < -12) t = -12;
  if (t >  12) t =  12;

  uint32_t mul = pgm_read_dword(&SEMI_MUL_Q16[t + 12]); // Q16
  uint32_t f   = ((uint32_t)freqHz * mul + 32768UL) >> 16;
