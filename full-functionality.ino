/*
  ...
  OLED behavior:
  - Base state (no song playing): alternates between 2 bitmap images every 2 seconds.
  - On button press (song start): show "Now playing <Title>" for 2 seconds,
    then return to alternating images (even while the song is playing).
  - If user stops/switches songs: it will still return to alternating images.

  Wiring:
  - Buttons: Ax to GND (INPUT_PULLUP, pressed = LOW)
  - Buzzer: D9
  - OLED: VCC, GND, SDA=A4, SCL=A5 (Nano)
...
const uint16_t TITLE_SHOW_MS = 2000;    // show "Now playing" for 2s
...
// OLED title overlay state
static bool titleActive = false;
static uint32_t titleUntilMs = 0;
static const char *titleLine2 = nullptr; // song title pointer
...
static void showNowPlaying(const char *songTitle) {
  titleActive = true;
  titleUntilMs = millis() + TITLE_SHOW_MS;
  titleLine2 = songTitle;
...
static void oledTick() {
  ...
  // If title overlay is active, keep it until time expires
  if (titleActive) {
    if ((int32_t)(now - titleUntilMs) >= 0) {
      titleActive = false;
      // resume alternating images immediately
      lastSwapMs = now;
...
static void handleSongButton(SongId requested) {
  ...
  // OLED title message (always show on press, even if it is a stop press)
  switch (requested) {
    case SONG_1: showNowPlaying("Space Oddity"); break;
    case SONG_2: showNowPlaying("Mandy"); break;
    case SONG_3: showNowPlaying("The Gambler"); break;
    default: break;
  }
}
...
void loop() {
  // Always keep OLED swapping (or showing title)
  oledTick();
